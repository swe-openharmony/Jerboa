...
| --------------------------------- | ------------------------------------------------------- | ------------------------------------------------ |
| `com.jerboa.hmos`         | HMOS bundle name (reverse-DNS, ends in `.hmos`)         | `com.beemdevelopment.aegis.hmos`                 |
| `Jerboa`              | Vendor / publisher name (usually app name)              | `Aegis`                                          |
| `1`        | Integer version code                                    | `1`                                              |
| `1.0.0`        | Semver version string                                   | `1.0.0`                                          |
| `Jerboa`           | Display name shown on the launcher                      | `Aegis`                                          |
| `jerboa`      | Lowercase, no-space identifier for `oh-package.json5`   | `aegis`                                          |
| `Jerboa Lemmy client for HarmonyOS` | One-line project description                            | `Aegis 2FA authenticator for HarmonyOS`          |
| `Jerboa main module`         | Description for `entry/src/main/module.json5` (module)  | `Aegis main module`                              |
| `Jerboa main entry ability`        | Description for `entry/src/main/module.json5` (ability) | `Aegis main entry ability`                       |
...
find "${DEST}" -type f \( -name '*.json5' -o -name '*.json' -o -name '*.ts' -o -name '*.ets' -o -name '*.md' \) \
  -exec sed -i \
    -e 's|com.jerboa.hmos|com.beemdevelopment.aegis.hmos|g' \
    -e 's|Jerboa|Aegis|g' \
    -e 's|1|1|g' \
    -e 's|1.0.0|1.0.0|g' \
    -e 's|Jerboa|Aegis|g' \
    -e 's|jerboa|aegis|g' \
    -e 's|Jerboa Lemmy client for HarmonyOS|Aegis 2FA authenticator for HarmonyOS|g' \
    -e 's|Jerboa main module|Aegis main module|g' \
    -e 's|Jerboa main entry ability|Aegis main entry ability|g' \
    {} +
```
...
$map = @{
  'com.jerboa.hmos'         = 'com.beemdevelopment.aegis.hmos'
  'Jerboa'              = 'Aegis'
  '1'        = '1'
  '1.0.0'        = '1.0.0'
  'Jerboa'           = 'Aegis'
  'jerboa'      = 'aegis'
  'Jerboa Lemmy client for HarmonyOS' = 'Aegis 2FA authenticator for HarmonyOS'
  'Jerboa main module'         = 'Aegis main module'
  'Jerboa main entry ability'        = 'Aegis main entry ability'
}
